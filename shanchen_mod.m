...
drho = 1;
delta_rho = -0.05*drho*(1-2.0*rand(lx,ly));
delta_rho = linspace(1-drho,1+drho,ly);
delta_rho = repmat( delta_rho, [ lx 1 ] );
...
for cycle = 1:maxT
    ...
    for i=1:9
        ...
        fOut(i,:,:)  = fIn(i,:,:) - omega1 .* (fIn(i,:,:)-fEq(i,:,:)) + omega1 .* rho1 .* tdgr( i );
    ...
    for i=1:9
        ...
        gOut(i,SOLID) = gIn(oppNS(i),SOLID);
    end
    ...
    rho1( SOLID ) = 0;
    ih.CData = rho1'; colorbar
    title('Fluid 1 density');
    axis equal off; drawnow( 'limitrate' )
    %end
end
